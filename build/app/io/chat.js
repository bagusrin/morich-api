...
module.exports.io = function (app, io, http, socket) {
	...
	socket.on('chatList', function (data) {
		axios.get(cfg.localhost + 'chat/list?email=' + data.email).then(response => {
			...
			if (response.data.success) {
				...
				axios.get(cfg.localhost + 'chat/list?email=' + data.sender).then(response => {
					...
					if (response.data.success) {
						...
						axios.get(cfg.localhost + 'chat/user-info?email=' + data.sender).then(response => {
				...
				axios.get(cfg.localhost + 'chat/list?email=' + data.receiver).then(response => {
