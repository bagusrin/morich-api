...
function cChat() {
  ...
  this.detail = function (req, res, next) {
    ...
    if (empty(conversationId)) return res.status(500).json({ statusCode: 500, message: "Parameter conversationId is required" });

    if (empty(requestBy)) return res.status(500).json({ statusCode: 500, message: "Parameter requestBy is required" });
